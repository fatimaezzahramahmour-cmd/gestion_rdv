...
def _is_patient(user):
    """Vérifie que l'utilisateur est un patient (rôle extranet)."""
    role = _get_role(user)
    return role == 'user'
...
def accueil(request):
    """Page d'accueil publique : cabinet dentaire. Connexion en haut à droite."""
...
def extranet(request):
    """
    EXTRANET : Espace privé réservé uniquement aux patients.
    Un agent ou admin qui tente d'accéder ici sera bloqué.
    """
    if not _is_patient(request.user):
        raise PermissionDenied  # → Page 403 Interdit pour agent/admin
...
def rdv_list(request):
    ...
    Agent → bloqué, redirigé vers son dashboard.
    """
    if _is_agent(request.user) and not _is_admin(request.user):
        return redirect('agent_dashboard')
...
def rdv_creneaux_api(request):
    """
    API créneaux disponibles pour une date.
...
def rdv_create(request):
    ...
    Réservé uniquement aux patients.
    """
    if not _is_patient(request.user):
        raise PermissionDenied
...
def rdv_patient_annuler(request, pk):
    """
    Annuler un RDV.
    Réservé uniquement aux patients propriétaires du RDV.
    """
    if not _is_patient(request.user):
...
def file_attente_view(request):
    """
    File d'attente patient : position numéro 1, 2, 3...
    Accessible aux patients uniquement.
    """
    if not _is_patient(request.user):
        raise PermissionDenied
...
def agent_dashboard(request):
    """
    Dashboard agent : RDV du jour, appeler prochain, valider/annuler.
    Réservé uniquement aux agents.
    """
    if not _is_agent(request.user):
        if _is_patient(request.user):
            return redirect('extranet')   # Patient → espace patient
        return redirect('/admin/')         # Admin → espace admin
...
def rdv_valider(request, pk):
    """
    Valider un RDV (marquer comme done).
    Réservé uniquement aux agents.
    """
    if not _is_agent(request.user):
        raise PermissionDenied
...
def rdv_annuler(request, pk):
    """
    Annuler un RDV via l'espace agent.
    Réservé uniquement aux agents.
    """
    if not _is_agent(request.user):
        raise PermissionDenied
...
def admin_dashboard(request):
    """
    Dashboard admin : statistiques, superviser RDV.
    Réservé uniquement aux admins (is_staff=True).
    """
    now = timezone.now()
